...
function ReportsPage() {
  ...
  useEffect(() => {
    const fetchClientsForFilter = async () => {
      try {
        ...
        setClients(clientsSnapshot.docs.map(d => ({ id: d.id, name: d.data().name })));
      } catch (error) { console.error("Error fetching clients for filter:", error); }
    };
    fetchClientsForFilter();
  }, []);

  // Corrected formatDateToISO
  ...
  const handleGenerateReport = async () => { 
    ...
    try {
      ...
      const conditions = [];
      if (selectedClientId) conditions.push(where('clientRef', '==', doc(db, 'clients', selectedClientId)));
      if (startDate) conditions.push(where('startTime', '>=', Timestamp.fromDate(new Date(startDate + "T00:00:00")))); // Ensure time is start of day
      if (endDate) conditions.push(where('startTime', '<=', Timestamp.fromDate(new Date(endDate + "T23:59:59")))); // Ensure time is end of day
      ...
      const entriesListPromises = querySnapshot.docs.map(async (docEntry) => {
        ...
        let clientName = 'N/A', categoryName = 'N/A', taskName = 'N/A', detailName = 'N/A', detailHourlyRate = null;
        if (entryData.clientRef) { try { const c = await getDoc(entryData.clientRef); if(c.exists()) clientName = c.data().name; } catch(e){console.error("Error fetching client for report entry:", e); clientName="Error"} }
        if (entryData.detailRef) { try {
            const d = await getDoc(entryData.detailRef); 
            if(d.exists()){ const dd = d.data(); detailName = dd.name || 'Unk Detail'; detailHourlyRate = dd.hourlyRate;
                const tR = d.ref.parent.parent; if(tR){ const t = await getDoc(tR); if(t.exists()){ taskName = t.data().name || 'Unk Task';
                    const cR = t.ref.parent.parent; if(cR){ const c = await getDoc(cR); if(c.exists()) categoryName = c.data().name || 'Unk Cat';}
                }}
            }} catch(e){console.error("Error fetching work item hierarchy for report entry:", e);}
        }
        currentTotalDuration += entryData.duration || 0;
        if (detailHourlyRate && typeof entryData.duration === 'number' && entryData.duration > 0) {
          const hours = (entryData.duration / 3600); currentTotalPrice += hours * parseFloat(detailHourlyRate);
        }
        return { id: docEntry.id, ...entryData, clientName, categoryName, taskName, detailName, detailHourlyRate, startTime: entryData.startTime?.toDate(), endTime: entryData.endTime?.toDate() };
      });
      ...
      setTotalCalculatedPrice(currentTotalPrice);
        ...
        if (error.code === 'failed-precondition' && error.message.includes('index')) { alert("A Firestore index is required. Check console for a link to create it.");
        } else { alert("Failed to generate report. See console."); }
        setFilteredTimeEntries([]); setTotalDurationSeconds(0); setTotalCalculatedPrice(0);
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = filteredTimeEntries.map(e => {
      ...
      return [ escape(justDate(e.startTime)), escape(e.clientName), escape(e.categoryName), escape(e.taskName), escape(e.detailName), escape(formatDuration(e.duration)), escape(justTime(e.startTime)), escape(justTime(e.endTime)), escape(e.detailHourlyRate !== undefined && e.detailHourlyRate !== null ? Number(e.detailHourlyRate).toFixed(2) : ''), escape(entryPrice) ].join(',');
    });
    ...
    if (selectedClientId) { const client = clients.find(c=>c.id === selectedClientId); if(client) filename+=`|${client.name.replace(/\s+/g,'_').replace(/[^a-zA-Z0-9_.-]/g,'')}`;
    } else { filename += `|AllClients`; }
    const sD = startDate?.replace(/-/g,'') || ''; const eD = endDate?.replace(/-/g,'') || '';
    if (sD && eD) filename += `_${sD}-${eD}`; else if (sD) filename += `_from-${sD}`; else if (eD) filename += `_until-${eD}`;
    filename += ".csv";
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ccc', borderRadius: '5px' }}>
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          <select id="clientFilter" value={selectedClientId} onChange={(e) => setSelectedClientId(e.target.value)} >
        ...
        <div style={{ marginTop: '15px' }}> {/* Added a div for better spacing of last row of buttons */}
